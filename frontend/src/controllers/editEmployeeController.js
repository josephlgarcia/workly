...
export async function initEditEmployeeEvents() {
    ...
    document.getElementById('edit-employeeForm').addEventListener("submit", async(event) => {
        ...
        try {
            const response = await api.putData(
                `http://localhost:3001/api/v1/employee/${employeeId}`,
                employeePayload
            );
            
            console.log("Respuesta del servidor:", response);

            alert(response.message || "Empleado editado con éxito ✅");
